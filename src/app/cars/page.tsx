...
export default function CarsPage() {
  ...
  useEffect(() => {
    fetch("/api/pieces")
      .then((r) => r.json())
      .then((data) => {
        const filtered = (data || []).filter((p: Piece) => p.category === "CAR");
        setPieces(filtered);
        setLoading(false);
      });
  }, []);
    ...
    <div className="min-h-screen relative overflow-hidden">
      ...
      <div className="relative z-10 pt-16">
        <div className="text-center pt-6 pb-3">
          ...
          <p className="text-sm md:text-base text-slate-400 italic">Legendary automobiles through history</p>
        </div>
        <div className="relative max-w-6xl mx-auto px-4 py-4">
          {loading || pieces.length === 0 ? (
            <div className="h-80 flex items-center justify-center text-slate-400">
              <p>{loading ? "Loading cars..." : "No cars found."}</p>
            </div>
          ) : (
            <>
              <div className="relative h-[320px] flex items-center justify-center gap-2 mb-3 overflow-hidden touch-pan-y"
