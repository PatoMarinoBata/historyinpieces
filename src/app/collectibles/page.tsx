    ...
    <div className="min-h-screen relative overflow-hidden">
      ...
      <div className="relative z-10 pt-16">
        <div className="text-center pt-6 pb-3">
          ...
          <p className="text-sm md:text-base text-slate-400 italic">Rare treasures and valuable artifacts</p>
        </div>
        <div className="relative max-w-6xl mx-auto px-4 py-4">
          {loading || pieces.length === 0 ? (
            <div className="h-80 flex items-center justify-center text-slate-400">
              <p>{loading ? "Loading collectibles..." : "No collectibles found."}</p>
            </div>
          ) : (
            <>
              <div
              ...
              <div className="relative max-w-xl mx-auto">
                ...
                <div key={`desc-${current?.id ?? currentIndex}-${animationKey}`} className={`desc-transition ${slideDirection} ${slideMode}`}>
                  <div className="bg-gradient-to-b from-slate-800 to-slate-900 rounded-lg border border-slate-700 p-4 shadow-lg">
                    ...
                    <Link href={`/pieces/${current?.id}`} className="inline-block bg-amber-600 hover:bg-amber-500 text-slate-900 px-6 py-3 rounded-lg font-semibold transition">
                      View Full Details
                    </Link>
                  </div>
                </div>
